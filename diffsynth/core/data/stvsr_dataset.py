        ...
        for key in self.data_file_keys:
            if key in data:
                if key in self.special_operator_map:
                    ...
                    data['GT'] = self.main_data_operator(data[key])
                    data['LQ'] = DownsampleVideoforIND(
                        space_scale=self.space_scale,
                    )(data['GT'])
        return data
    

class OODDataset(UnifiedDataset):
    def __init__(
        self,
        base_path=None, metadata_path=None,
        repeat=1,
    ...
    def load_clip_operators(
        ...
        return RouteByType(operator_map=[
            # new metadata entrance：dict -> LoadClip
            (dict, LoadClip(
                base_path=base_path,
                default_k=default_k,
                frame_processor=frame_proc,
        ...
        for key in self.data_file_keys:
            if key in data:
                if key in self.special_operator_map:
                    ...
                    data['GT'] = self.main_data_operator(data[key])
                    frame_indices = [0, 1, -3, -2]
                    data['LQ'] = [data['GT'][i] for i in frame_indices]
                    data['GT'] = data['GT'][-1:]
        return data